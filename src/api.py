...
from src.utils import calculate_individual_kpi, calculate_team_kpi, filter_features, calculate_team_and_individual_kpis
from src.preprocess import extract_features_from_project, extract_individual_metrics, parse_date
...
def calculate_developer_metrics(commits: List[Dict]) -> Dict:
    """Вычисляет метрики для индивидуального разработчика"""
    total = len(commits)
    if total == 0:
        return {
    ...
    # Считаем типы коммитов с улучшенной классификацией
    feature_commits = sum(1 for c in commits if any(word in c.get("message", "").lower()
                                                    for word in ["feat", "feature", "add", "implement", "new"]))
    fix_commits = sum(1 for c in commits if any(word in c.get("message", "").lower()
                                                for word in ["fix", "bug", "error", "issue", "resolve"]))
    refactor_commits = sum(1 for c in commits if any(word in c.get("message", "").lower()
                                                     for word in ["refactor", "cleanup", "optimize", "improve"]))
    test_commits = sum(1 for c in commits if any(word in c.get("message", "").lower()
                                                 for word in ["test", "spec", "coverage"]))
    docs_commits = sum(1 for c in commits if any(word in c.get("message", "").lower()
                                                 for word in ["doc", "readme", "comment", "document"]))
    ...
    return {
        "feature_ratio": feature_commits / total,
